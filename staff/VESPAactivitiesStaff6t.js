                    ...
                    try {
                        ...
                        if (responseData && typeof responseData === 'object') {
                            ...
                            if (responseData.responses) {
                                ...
                                // Handle cycle format
                                parsedResponses = [{ response: responseData.cycle_1.value }];
                            } else {
                                // Handle key-value pairs as individual responses
                                parsedResponses = Object.entries(responseData).map(([key, value]) => ({
                                    question: key,
                                    response: value && typeof value === 'object' ? value.value || JSON.stringify(value) : value
                                }));
                            }
                        }
                    } catch (e) {
                                            ...
                                            <div class="responses-list" style="background: #e8f4fd; padding: 16px; border-radius: 6px; border-left: 4px solid #17a2b8;">
                                                ${parsedResponses.map((response, index) => `
                                                    <div class="response-item" style="margin-bottom: 16px; ${index < parsedResponses.length - 1 ? 'border-bottom: 1px solid #b8daff; padding-bottom: 16px;' : ''}">
                                                        ${response.question ? `<div class="response-question" style="font-weight: 500; color: #495057; margin-bottom: 8px;">${this.escapeHtml(response.question)}</div>` : ''}
                                                        <div class="response-content" style="background: white; padding: 12px; border-radius: 4px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.5; color: #212529; border: 1px solid #b8daff; font-weight: 600;">
                                                            ${this.escapeHtml(response.response || response.value || response)}
                                                        </div>
                                                    </div>
                                                `).join('')}
                                            </div>
                                    ...
                                    <div class="feedback-section" style="background: #f1f8ff; padding: 20px; border-radius: 8px; border: 2px solid #007bff;">
                                        ...
                                        <div class="feedback-input-container">
                                            <textarea 
                                                ...
                                                placeholder="Enter your feedback for the student here. This will be visible to the student when they next access their activities."
                                                rows="6"
