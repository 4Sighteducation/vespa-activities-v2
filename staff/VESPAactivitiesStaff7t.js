...
(function() {
    ...
    class VESPAStaffManager {
            ...
            try {
                ...
                while (hasMorePages) {
                    ...
                    const requestData = {
                        ...
                        page: page,
                        rows_per_page: 100, // Reduced from 1000 to improve performance
        ...
        async loadVESPAScores() {
            ...
            try {
                ...
                if (allVespaRecords.length > 0) {
                    ...
                    allVespaRecords.forEach(record => {
                    ...
                    log(`Loaded VESPA scores for ${allVespaRecords.length} records`);
        ...
        async showActivitySeriesModal(studentId, bulkMode = false) {
            try {
                ...
                const selectedCount = bulkMode ? Array.from(this.state.selectedStudents).length : 1;
                const headerText = bulkMode ? 
                    `🎯 Activity Series - Assigning to ${selectedCount} Student${selectedCount > 1 ? 's' : ''}` :
                    '🎯 Activity Series - Problem-Based Selection';
                
                const modalHtml = `
                    <div class="modal-overlay" id="activity-series-modal" data-bulk-mode="${bulkMode}" data-student-id="${studentId}" onclick="VESPAStaff.closeActivitySeriesModal()">
                        <div class="modal-content activity-series-modal" onclick="event.stopPropagation()">
                            ...
                            <div class="modal-body">
                                <div class="problems-header">
                                    <p>Select a challenge ${bulkMode ? 'the students are' : 'the student is'} facing to get targeted activity recommendations:</p>
                                </div>
        ...
        showActivitySelectionModal(problem, activities, studentIds) {
            // Handle both single student and bulk mode
            const isBulk = Array.isArray(studentIds);
            const studentList = isBulk ? studentIds : [studentIds];
            const studentCount = studentList.length;
            const thirdPersonText = this.convertToThirdPerson(problem.text);
            
            const headerText = studentCount > 1 ? 
                `💡 Recommended Activities for ${studentCount} Students` :
                '💡 Recommended Activities';
            
            const modalHtml = `
                <div class="modal-overlay" id="activity-selection-modal" data-student-ids="${studentList.join(',')}" onclick="VESPAStaff.closeActivitySelectionModal()">
                    <div class="modal-content activity-selection-modal" onclick="event.stopPropagation()">
                        ...
                        <div class="modal-body">
                            <div class="problem-context">
                                ...
                                ${studentCount > 1 ? `<p><em>Assigning to ${studentCount} students</em></p>` : ''}
                            </div>
        ...
        async addSelectedActivities(studentIdParam) {
            // Get student IDs from modal data attribute
            const modal = document.getElementById('activity-selection-modal');
            const studentIds = modal ? 
                modal.dataset.studentIds?.split(',').filter(Boolean) : 
                [studentIdParam];
        ...
        async replaceAllActivities(studentIdParam) {
            // Get student IDs from modal data attribute
            const modal = document.getElementById('activity-selection-modal');
            const studentIds = modal ? 
                modal.dataset.studentIds?.split(',').filter(Boolean) : 
        ...
        renderActivityList() {
            ...
            return `
                <div class="activity-filters">
                    ...
                    <button class="btn btn-primary" onclick="VESPAStaff.showBulkActivitySeriesModal()">
                        🎯 Activity Series
                    </button>
                </div>
        ...
        async showBulkActivitySeriesModal() {
            ...
            // Close the assign modal if open
            this.closeModal('assign-modal');
            
            // Show Activity Series modal for bulk assignment
            // We'll use the first student as a representative for the problem selection
            const studentId = selectedStudents[0];
            await this.showActivitySeriesModal(studentId, true); // true indicates bulk mode
        }
